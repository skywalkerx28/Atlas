...
The Agent Sessions Workbench (`Workbench` in `sessions/browser/workbench.ts`) provides a simplified, fixed layout optimized for agent session workflows. Unlike the default workbench, this layout:
...
| Open... | (submenu) | Right toolbar (`TitleBarRight`) | Split button submenu: Open Terminal, Open in Atlas |
...
- Contains "Open Terminal" (opens terminal at session worktree) and "Open in Atlas" (opens worktree in new Atlas window)
...
│   │   ├── chat.contribution.ts            # Open in Atlas, Open Terminal, branch chat, run script, prompts service
