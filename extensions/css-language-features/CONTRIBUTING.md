...
- Open `/extensions/css-language-features/` as the workspace in Atlas
- In `/extensions/css-language-features/` run `npm run compile`(or `npm run watch`) to build the client and server
- Run the [`Launch Extension`](https://github.com/skywalkerx28/Atlas/blob/master/extensions/css-language-features/.vscode/launch.json) debug target in the Debug View. This will:
	- Launch a new Atlas instance with the `css-language-features` extension loaded
...
- Debug the language server process by using `Attach to Node Process` command in the  Atlas window opened on `css-language-features`.
...
This extension wraps the css language service into a Language Server for Atlas.
...
- Open both `vscode-css-languageservice` and this extension in a single workspace with [multi-root workspace](https://github.com/skywalkerx28/Atlas) feature
