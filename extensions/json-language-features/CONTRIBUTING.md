...
- Open `/extensions/json-language-features/` as the workspace in Atlas
- In `/extensions/json-language-features/` run `npm run compile`(or `npm run watch`) to build the client and server
- Run the [`Launch Extension`](https://github.com/skywalkerx28/Atlas/blob/master/extensions/json-language-features/.vscode/launch.json) debug target in the Debug View. This will:
	- Launch a new Atlas instance with the `json-language-features` extension loaded
...
- Debug the language server process by using `Attach to Node Process` command in the  Atlas window opened on `json-language-features`.
...
- Open both `vscode-json-languageservice` and this extension in two windows or with a single window with the[multi-root workspace](https://github.com/skywalkerx28/Atlas) feature.
