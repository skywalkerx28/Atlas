...
- Clone [skywalkerx28/Atlas](https://github.com/skywalkerx28/Atlas)
...
- Open `/extensions/html-language-features/` as the workspace in Atlas
- In `/extensions/html-language-features/` run `npm run compile`(or `npm run watch`) to build the client and server
- Run the [`Launch Extension`](https://github.com/skywalkerx28/Atlas/blob/master/extensions/html-language-features/.vscode/launch.json) debug target in the Debug View. This will:
	- Launch a new Atlas instance with the `html-language-features` extension loaded
...
- Debug the language server process by using `Attach to Node Process` command in the  Atlas window opened on `html-language-features`.
...
This extension wraps the html language service into a Language Server for Atlas.
...
- Open both `vscode-html-languageservice` and this extension in two windows or with a single window with the[multi-root workspace](https://github.com/skywalkerx28/Atlas) feature
