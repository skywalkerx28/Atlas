---
name: launch
description: "Launch and automate Atlas using agent-browser via Chrome DevTools Protocol. Use when you need to interact with the Atlas UI, automate the chat panel, test UI features, or take screenshots of Atlas. Triggers include 'automate Atlas', 'interact with chat', 'test the UI', 'take a screenshot', 'launch Atlas with debugging'."
metadata:
...
Automate Atlas using agent-browser. Atlas is built on Electron/Chromium and exposes a Chrome DevTools Protocol (CDP) port that agent-browser can connect to, enabling the same snapshot-interact workflow used for web pages.
...
- **For Atlas (dev build):** The repo must be built before launching. `./scripts/code.sh` runs the build automatically if needed, or set `VSCODE_SKIP_PRELAUNCH=1` to skip the compile step if you've already built.
- **CSS selectors are internal implementation details.** Selectors like `.interactive-input-part`, `.interactive-input-editor`, and `.part.auxiliarybar` used in `eval` commands are Atlas internals that may change across versions. If they stop working, use `agent-browser snapshot -i` to re-discover the current DOM structure.

## Core Workflow

1. **Launch** Atlas with remote debugging enabled
...
> SCREENSHOT_DIR="/tmp/atlas-screenshots/$(date +%Y-%m-%dT%H-%M-%S)"
...
```bash
# Launch Atlas with remote debugging
./scripts/code.sh --remote-debugging-port=9224

# Wait for Atlas to start, retry until connected
...
## Launching Atlas (Dev Build)

The Atlas repository includes `scripts/code.sh` which launches Atlas from source. It passes all arguments through to the Electron binary, so `--remote-debugging-port` works directly:

```bash
cd <repo-root>  # the root of your Atlas checkout
./scripts/code.sh --remote-debugging-port=9224
```
...
- Atlas uses the default user data directory. Unlike VS Code Insiders, you don't typically need `--user-data-dir` since there's usually only one Atlas instance running.
- If you see "Sent env to running instance. Terminating..." it means Atlas is already running and forwarded your args to the existing instance. Quit Atlas and relaunch with the flag, or use `--user-data-dir=/tmp/atlas-debug` to force a new instance.

## Launching Extensions for Debugging

To debug an extension via agent-browser, launch VS Code Insiders with `--extensionDevelopmentPath` and `--remote-debugging-port`. Use `--user-data-dir` to avoid conflicting with an already-running instance.
...
# Wait for VS Code Insiders to start, retry until connected
...
- `--user-data-dir=<path>` — uses a separate profile so it starts a new process instead of sending to an existing instance

**Without `--user-data-dir`**, VS Code Insiders detects the running instance, forwards the args to it, and exits immediately — you'll see "Sent env to running instance. Terminating..." and CDP never starts.

## Restarting After Code Changes

**After making changes to Atlas source code, you must restart to pick up the new build.** The workbench loads the compiled JavaScript at startup -- changes are not hot-reloaded.

### Restart Workflow

1. **Rebuild** the changed code
2. **Kill** the running Atlas instance
...
# 2. Kill the Atlas instance listening on the debug port (if running)
pids=$(lsof -t -i :9224)
if [ -n "$pids" ]; then
	kill $pids
fi
...
> **Tip:** If you're iterating frequently, run `npm run watch` in a separate terminal so compilation happens automatically. You still need to kill and relaunch Atlas to load the new build.

## Interacting with Monaco Editor (Chat Input, Code Editors)

Atlas uses Monaco Editor for all text inputs including the Copilot Chat input. Monaco editors require specific agent-browser techniques — standard `click`, `fill`, and `keyboard type` commands may not work depending on the Atlas build.

### The Universal Pattern: Focus via Keyboard Shortcut + `press`

This works on **all** Atlas builds (dev, Insiders, stable):
...
This shortcut focuses the chat input and sets `document.activeElement` to a `DIV` with class `native-edit-context` — Atlas's native text editing surface that correctly processes key events from `agent-browser press`.
...
However, **`type @ref` silently fails on Atlas** — the command completes without error but no text appears. This also applies to `keyboard type` and `keyboard inserttext`. Always verify text appeared after typing, and fall back to the keyboard shortcut + `press` pattern if it didn't. The `press`-per-key approach works universally across all builds.
...
- Make sure Atlas was launched with `--remote-debugging-port=NNNN`
- If Atlas was already running, quit and relaunch with the flag
...
- Atlas uses multiple webviews. Use `agent-browser tab` to list targets and switch to the right one
...
- `type @ref`, `keyboard type`, and `keyboard inserttext` work on VS Code Insiders but **silently fail on Atlas** — they complete without error but no text appears. The `press`-per-key approach works universally.
- See the "Interacting with Monaco Editor" section above for the full compatibility matrix.

## Cleanup

**Always kill the Atlas instance when you're done.** Atlas is a full Electron app that consumes significant memory (often 1–4 GB+). Leaving it running wastes resources and holds the CDP port.

```bash
# Disconnect agent-browser
agent-browser close

# Kill the Atlas instance listening on the debug port (if running)
# macOS / Linux:
pids=$(lsof -t -i :9224)
if [ -n "$pids" ]; then
	kill $pids
...
# Windows:
# taskkill /F /PID <PID>
# Or use Task Manager to end "Atlas"
```
